...
public class NewElectionModel : PageModel
{
    ...
    private readonly ILogger<NewElectionModel> _logger;
    private readonly string _connectionString;
    private readonly string BaseUploadPath;
    private readonly string StatusFilePath;

    public NewElectionModel(IConfiguration config, IWebHostEnvironment env, ILogger<NewElectionModel> logger)
{
    _config = config;
    _env = env;
    _logger = logger;
...
System.Diagnostics.Debug.WriteLine($"StatusFilePath set to: {StatusFilePath}");
    ...
    public async Task<IActionResult> OnPostUploadAsync()
    {
        ...
        if (Request.Form.TryGetValue("IsPrimary", out var isPirmaryVal))
        {
            IsPrimary = isPirmaryVal.Contains("true", StringComparer.OrdinalIgnoreCase);
        }
